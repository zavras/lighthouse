...
const UIStrings = {
  ...
  failureTitle: 'Prevents users to paste into input fields',
  ...
  description: 'Preventing input pasting undermines good security policy (in ' +
      'the case of passwords) and is a UX anti-pattern (in the general case). ' +
      '[Learn more about user-friendly input fields](https://web.dev/password-inputs-can-be-pasted-into/).',
};
...
class PasswordInputsCanBePastedIntoAudit extends Audit {
  ...
  static audit(artifacts) {
    const inputsWithPreventedPaste = artifacts.Inputs.inputs.filter(input => input.preventsPaste);

    /** @type {LH.Audit.Details.Table['items']} */
    const items = [];

    inputsWithPreventedPaste.forEach(input => {
      items.push({
    ...
    return {
      score: Number(inputsWithPreventedPaste.length === 0),
